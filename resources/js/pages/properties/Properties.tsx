...
export default function Properties() {
    ...
    const filtersProp =
        ...
        !Array.isArray(rawFilters)
            ...
            : {};
    const filterString = (key: string) => {
        const value = filtersProp[key];
        return typeof value === 'string' || typeof value === 'number'
            ? String(value)
            : '';
    };
    const allowedSorts = ['newest', 'low_price', 'high_price'];
    const initialSort =
        typeof filtersProp.sort === 'string' &&
        allowedSorts.includes(filtersProp.sort)
            ? filtersProp.sort
            : 'newest';
    const initialAmenities = Array.isArray(filtersProp.amenities)
    ...
    const [municipalityId, setMunicipalityId] = useState(
        filterString('municipality_id'),
    );
    const [priceMin, setPriceMin] = useState(filterString('price_min'));
    const [priceMax, setPriceMax] = useState(filterString('price_max'));
    const [bedrooms, setBedrooms] = useState(filterString('bedrooms'));
    const [bathrooms, setBathrooms] = useState(filterString('bathrooms'));
    const [amenities, setAmenities] = useState<number[]>(initialAmenities);
    ...
    const { ads: properties, loading } = useAds(
        currentFilters,
        props.properties,
    );
    const resultTotal =
    ...
    return (
        <App>
            ...
            <div className="min-h-screen bg-[#EEEFE6] pb-20 dark:bg-[#292625]">
                    ...
                    <div className="mb-6 overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
                        ...
                        <div className="flex flex-wrap items-center gap-0 divide-x divide-gray-100">
                            ...
                            <div className="relative min-w-[160px]">
                                <Home className="pointer-events-none absolute top-1/2 left-4 h-4 w-4 -translate-y-1/2 text-[#CF8E19]" />
                                <select
                                    id="sale-type-filter"
                                    aria-label={
                                        t('status') || 'Type de transaction'
                                    }
                                    value={saleType}
                                    onChange={(e) =>
                            ...
                            {/* Localisation */}
                            <div className="relative min-w-[160px]">
                                <MapPin className="pointer-events-none absolute top-1/2 left-4 h-4 w-4 -translate-y-1/2 text-[#CF8E19]" />
                                <select
                                    id="municipality-filter"
                                    aria-label={t('location') || 'Localisation'}
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2.5 px-6 py-4 text-sm font-semibold transition-colors duration-200 ${
                                    filtersOpen || activeFilterCount > 0
                                        ? 'bg-[#413D3C] text-white dark:bg-[#CF8E19] dark:text-[#292625]'
                            ...
                            >
                                ...
                                {activeFilterCount > 0 && (
                                    <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-[#CF8E19] px-1.5 text-xs font-bold text-[#292625]">
                                        {activeFilterCount}
                                    </span>
                                )}
                            </button>
                        ...
                        >
                            <div className="border-t border-gray-100 p-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <label
                                                ...
                                                className="mb-1.5 flex items-center gap-1 text-xs font-semibold tracking-wide text-gray-500 uppercase"
                                            >
                                ...
                                {allAmenities.length > 0 && (
                                    <div className="mt-5">
                                        <div
                                            id="amenities-filter-label"
                                            className="mb-3 block text-xs font-semibold tracking-wide text-gray-500 uppercase"
                                        >
                                        ...
                                        <div
                                            ...
                                            role="group"
                                            aria-labelledby="amenities-filter-label"
                                        >
                                                ...
                                                <button
                                                    ...
                                                    className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-all duration-200 ${
                                                        amenities.includes(a.id)
                                                            ? 'border-[#413D3C] bg-[#413D3C] text-white dark:border-[#CF8E19] dark:bg-[#CF8E19] dark:text-[#292625]'
                                                            : 'border-gray-200 bg-white text-gray-600 hover:border-[#413D3C]/40 hover:text-[#413D3C] dark:bg-[#353130] dark:text-[#EEEFE6]/70 dark:hover:border-[#CF8E19]'
                                                    }`}
                                                >
                                                    {a.name}
                                                </button>
                                ...
                                <div className="mt-5 flex items-center justify-between border-t border-gray-100 pt-5">
                                    ...
                                    <button
                                        ...
                                        className="rounded-xl bg-[#413D3C] px-5 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#292625] dark:bg-[#CF8E19] dark:text-[#292625] dark:hover:bg-[#E0A43A]"
                                    >
                                        Appliquer
                                    </button>
                                </div>
                        ...
                        >
                            {loading ? (
                                <span className="inline-flex items-center">
                                    ...
                                    <span
                                        aria-hidden="true"
                                        className="skeleton block h-4 w-36 rounded-md"
                                    />
                                </span>
                            ) : (
                                <>
                                    ...
                                    {t('properties_found')}
                                    {debouncedSearch && (
                                        <span className="font-semibold text-[#413D3C] dark:text-[#CF8E19]">
                                            {' '}
                                            {t('results_for_query', {
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <button
                                    ...
                                    aria-label={
                                        t('grid_view') || 'Vue en grille'
                                    }
                                    aria-pressed={viewMode === 'grid'}
                                    className={`rounded-lg p-2 transition-all duration-200 ${viewMode === 'grid' ? 'bg-[#413D3C] text-white shadow-sm dark:bg-[#CF8E19] dark:text-[#292625]' : 'text-gray-400 hover:text-gray-600'}`}
                                    title={t('grid_view')}
                                >
                                    <Grid size={16} />
                                </button>
                                <button
                                    ...
                                    aria-label={
                                        t('list_view') || 'Vue en liste'
                                    }
                                    aria-pressed={viewMode === 'list'}
                                    className={`rounded-lg p-2 transition-all duration-200 ${viewMode === 'list' ? 'bg-[#413D3C] text-white shadow-sm dark:bg-[#CF8E19] dark:text-[#292625]' : 'text-gray-400 hover:text-gray-600'}`}
                                    title={t('list_view')}
                                >
                                    <List size={16} />
                                </button>
