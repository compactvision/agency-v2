...
export default function Pricing({
    ...
    const [showSellerRequired, setShowSellerRequired] = useState(false);
    const [showSellerPopup, setShowSellerPopup] = useState(false);
    const canSubscribe = (auth?.user?.roles ?? []).some(
        (role: string | { name: string }) =>
            ['seller', 'agency', 'admin'].includes(
                typeof role === 'string' ? role : role.name,
            ),
    );

    const { subscribe, submittingPlan, serverErrors, globalError } =
    ...
    const isPopularPlan = (index: number) => {
    ...
    return (
        <>
            ...
            >
                <DialogContent>
                    <DialogTitle>Devenez vendeur pour vous abonner</DialogTitle>
                    <DialogDescription>
                        Votre compte est un compte acheteur. Vous devez d’abord
                        devenir vendeur pour pouvoir souscrire un abonnement.
                    </DialogDescription>
                    <DialogFooter>
                        <button
                            type="button"
                            onClick={() => setShowSellerRequired(false)}
                        ...
                        <button
                            type="button"
                            onClick={() => {
                                setShowSellerRequired(false);
                                setShowSellerPopup(true);
                            }}
                            className="rounded-lg bg-[#CF8E19] px-4 py-2 font-semibold text-[#292625] hover:bg-[#E0A43A]"
                        >
                            Devenir vendeur
                        </button>
                    </DialogFooter>
            ...
            <section
                id="tarifs"
                className="relative scroll-mt-24 overflow-hidden bg-gradient-to-br from-[#F7F7F2] via-[#EEEFE6] to-[#E4E5DC] py-20 dark:from-[#211F1E] dark:via-[#292625] dark:to-[#211F1E]"
            >
                ...
                <div className="relative z-10 container mx-auto px-4">
                    ...
                    {plans.length === 0 && (
                        <p className="text-center text-[#625E5B] dark:text-[#C8C2BA]">
                            Nos offres seront bientôt disponibles.
